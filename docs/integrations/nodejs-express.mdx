...
export async function getAllUsers(
  ...
  return tryAsync({
    ...
    catch: (error) => UserServiceErr({ operation: 'getAllUsers' }),
  });
}

export async function getUserById(id: string): Promise<Result<User, UserServiceError>> {
  if (!id) {
    return UserServiceErr({ operation: 'getUserById', userId: id });
  }
  ...
  return tryAsync({
    ...
    catch: (error) => UserServiceErr({ operation: 'getUserById', userId: id }),
  });
}
...
export async function updateUser(id: string, input: UpdateUserInput): Promise<Result<User, UserServiceError>> {
  ...
  return tryAsync({
    ...
    catch: (error) => UserServiceErr({ operation: 'updateUser', userId: id }),
  });
}

export async function deleteUser(id: string): Promise<Result<void, UserServiceError>> {
  if (!id) {
    return UserServiceErr({ operation: 'deleteUser', userId: id });
  }
  ...
  return tryAsync({
    ...
    catch: (error) => UserServiceErr({ operation: 'deleteUser', userId: id }),
  });
}
...
const validationErrors = defineErrors({
  ValidationError: ({ location }: { location: string }) => ({
...
export function validateBody<T>(schema: z.ZodSchema<T>) {
  return (req: Request, res: Response, next: NextFunction) => {
    const { data: validData, error } = trySync({
      ...
      catch: (error) => ValidationErr({ location: 'body' }),
    });
...
export function validateParams<T>(schema: z.ZodSchema<T>) {
  return (req: Request, res: Response, next: NextFunction) => {
    const { data: validParams, error } = trySync({
      ...
      catch: (error) => ValidationErr({ location: 'params' }),
    });
...
export function validateQuery<T>(schema: z.ZodSchema<T>) {
  return (req: Request, res: Response, next: NextFunction) => {
    const { data: validQuery, error } = trySync({
      ...
      catch: (error) => ValidationErr({ location: 'query' }),
    });
...
const databaseErrors = defineErrors({
  DatabaseError: ({ operation }: { operation: string }) => ({
    message: `Database ${operation} failed`,
    operation,
  }),
});
const { DatabaseError, DatabaseErr } = databaseErrors;
type DatabaseError = InferError<typeof databaseErrors, 'DatabaseError'>;
...
class Database {
  ...
  async connect(): Promise<Result<void, DatabaseError>> {
    return tryAsync({
      ...
      catch: (error) => DatabaseErr({ operation: 'connect' }),
    });
  }
  ...
  async disconnect(): Promise<Result<void, DatabaseError>> {
    return tryAsync({
      ...
      catch: (error) => DatabaseErr({ operation: 'disconnect' }),
    });
  }
  ...
  async healthCheck(): Promise<Result<{ status: string; timestamp: Date }, DatabaseError>> {
    return tryAsync({
      ...
      catch: (error) => DatabaseErr({ operation: 'healthCheck' }),
    });
  }
...
const authErrors = defineErrors({
  AuthError: ({ reason }: { reason: string }) => ({
    message: `Authentication failed: ${reason}`,
    reason,
  }),
});
const { AuthError, AuthErr } = authErrors;
type AuthError = InferError<typeof authErrors, 'AuthError'>;
  ...
  describe('GET /api/users', () => {
    ...
    it('handles service errors gracefully', async () => {
      vi.mocked(userService.getAllUsers).mockResolvedValue(Err(
        UserServiceError({ operation: 'getAllUsers' })
      ));
  ...
  describe('POST /api/users', () => {
    ...
    it('handles duplicate email error', async () => {
      ...
      vi.mocked(userService.createUser).mockResolvedValue(Err(
        UserServiceError({ operation: 'createUser' })
      ));
  ...
  describe('GET /api/users/:id', () => {
    ...
    it('returns 404 for non-existent user', async () => {
      vi.mocked(userService.getUserById).mockResolvedValue(Err(
        UserServiceError({ operation: 'getUserById', userId: 'non-existent' })
      ));
...
const configErrors = defineErrors({
  ConfigError: ({ reason }: { reason: string }) => ({
    message: `Configuration error: ${reason}`,
    reason,
  }),
});
const { ConfigError, ConfigErr } = configErrors;
type ConfigError = InferError<typeof configErrors, 'ConfigError'>;
...
// ✅ Good - consistent format
{
  ...
  "error": {
    "name": "UserServiceError",
    "message": "User service getUserById failed",
