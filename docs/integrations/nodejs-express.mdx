...
const { UserServiceError, UserServiceErr } = createTaggedError('UserServiceError')
  .withFields<{ operation: string; userId?: string }>()
  .withMessage(({ operation }) => `User service ${operation} failed`);
...
const { ValidationError, ValidationErr } = createTaggedError('ValidationError')
  .withFields<{ location: string }>()
  .withMessage(({ location }) => `Validation failed for ${location}`);
...
const { AuthError, AuthErr } = createTaggedError('AuthError')
  ...
  .withMessage(({ reason }) => `Authentication failed: ${reason}`);
...
const { ConfigError, ConfigErr } = createTaggedError('ConfigError')
  .withFields<{ reason: string }>()
  .withMessage(({ reason }) => `Configuration error: ${reason}`);
