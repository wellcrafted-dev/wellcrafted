...
const UserNotFoundError = defineErrors({
  NotFound: ({ userId }: { userId: string }) => ({
    message: `User ${userId} not found`,
    userId,
  }),
});
type UserNotFoundError = InferErrors<typeof UserNotFoundError>;

async function getUser(id: string): Promise<Result<User, UserNotFoundError>> {
  if (!id) {
    return UserNotFoundError.NotFound({ userId: id });
  }
...
forEachAction(client, ({ workspaceId, actionName, action }) => {
  ...
  app.get(path, async (c) => {
    ...
    const maybeResult = await action(input) as Result<unknown, AnyTaggedError> | unknown;
