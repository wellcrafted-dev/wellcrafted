...
📊 Results: 97% code sharing • 0 runtime crashes • 22,824 lines
```
...
<script lang="ts">
  ...
  async function handleDelete(id: string) {
    ...
    // Or shorthand: await rpc.recordings.deleteRecording(id)
    if (error) {
...
const recordingErrors = defineErrors({
  RecordingServiceError: ({ operation, recordingId }: { operation: string; recordingId?: string }) => ({
    message: `Recording ${operation} failed`,
    operation,
    recordingId,
  }),
});
const { RecordingServiceError, RecordingServiceErr } = recordingErrors;
type RecordingServiceError = InferError<typeof recordingErrors, 'RecordingServiceError'>;
  ...
  return {
    async getAllRecordings(): Promise<Result<Recording[], RecordingServiceError>> {
      return tryAsync({
        ...
        catch: (error) => RecordingServiceErr({ operation: 'getAllRecordings' }),
      });
    },

    async getRecording(id: string): Promise<Result<Recording, RecordingServiceError>> {
      if (!id) {
        return RecordingServiceErr({ operation: 'getRecording', recordingId: id });
      }
      ...
      return tryAsync({
        ...
        catch: (error) => RecordingServiceErr({ operation: 'getRecording', recordingId: id }),
      });
    },
    ...
    async createRecording(input: CreateRecordingInput): Promise<Result<Recording, RecordingServiceError>> {
      ...
      if (!input.blob || input.blob.size === 0) {
        return RecordingServiceErr({ operation: 'createRecording' });
      }
      ...
      return tryAsync({
        ...
        catch: (error) => RecordingServiceErr({ operation: 'createRecording' }),
      });
    },

    async deleteRecording(id: string): Promise<Result<void, RecordingServiceError>> {
      if (!id) {
        return RecordingServiceErr({ operation: 'deleteRecording', recordingId: id });
      }
      ...
      return tryAsync({
        ...
        catch: (error) => RecordingServiceErr({ operation: 'deleteRecording', recordingId: id }),
      });
    },
    ...
    async updateRecording(id: string, updates: Partial<Recording>): Promise<Result<Recording, RecordingServiceError>> {
      return tryAsync({
        ...
        catch: (error) => RecordingServiceErr({ operation: 'updateRecording', recordingId: id }),
      });
    },
  };
}
...
export const recordings = {
  ...
  getAllRecordings: defineQuery({
    queryKey: ['recordings'],
    queryFn: () => recordingService.getAllRecordings(),
  ...
  createRecording: defineMutation({
    ...
    mutationFn: async (input: CreateRecordingInput) => {
...
<script lang="ts">
  ...
  async function handleTranscribeAndEdit() {
    ...
    // Or shorthand: await rpc.transcription.transcribeRecording(recording.id)
    if (transcribeError) {
  ...
  async function handleDelete() {
    ...
    // Or shorthand: await rpc.recordings.deleteRecording(recording.id)
    if (error) {
  ...
  async function handleShare() {
    ...
    // Or shorthand: await rpc.recordings.generateShareLink(recording.id)
    if (error) {
...
<script lang="ts">
  ...
  // Complex workflow: Record → Save → Transcribe
  async function handleStartRecording() {
    const toastId = nanoid();
    
    const { data, error } = await rpc.recorder.startRecording.execute({ toastId });
    // Or shorthand: await rpc.recorder.startRecording({ toastId })
    if (error) {
  ...
  async function handleStopAndTranscribe() {
    ...
    // Or shorthand: await rpc.recorder.stopRecording({ toastId })
    if (stopError) {
    ...
    // Or shorthand: await rpc.recordings.createRecording({ ... })
    if (saveError) {
    ...
    if (transcribeError) {
      toast.error('Recording saved, but transcription failed', { 
        ...
        action: {
          ...
          onClick: () => rpc.transcription.transcribeRecording(recording.id)
        }
      });
      return;
    }
  ...
  onDestroy(() => {
    if (isRecording) {
      ...
      // Or shorthand: rpc.recorder.stopRecording({ toastId: nanoid() })
    }
  });
</script>
...
export const load: PageLoad = async () => {
  ...
  // Or shorthand: await rpc.recordings.getAllRecordings()
...
export async function getAllRecordings(): Promise<Result<Recording[], RecordingServiceError>> {
  return tryAsync({
    ...
    catch: (error) => RecordingServiceErr({
      operation: 'getAllRecordings',
    }),
  });
}
...
<script lang="ts">
  async function handleCreateRecording() {
    ...
    // Or shorthand: await rpc.recordings.createRecording(formData)
...
│   ├── CreateRecordingForm.svelte
│   └── DeleteRecordingDialog.svelte
├── transcription/
│   ├── TranscriptionViewer.svelte
│   ├── TranscriptionEditor.svelte
│   └── TranscriptionProgress.svelte
└── shared/
    ├── ErrorDisplay.svelte
