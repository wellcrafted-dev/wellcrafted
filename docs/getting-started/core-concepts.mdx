...
1. **Discriminated Union**: The `error` property is the sole discriminant. `error === null` means Ok, `error !== null` means Err. Why not check `data`? Because `data` can be `null` as a valid success value (`Ok<null>`), so it's not a universal discriminant
...
type TaggedError<T extends string> = {
  ...
  cause?: TaggedError;                 // Root cause (another TaggedError)
}
```
