...
// Define our error types with defineErrors
// One namespace groups all related variants
...
type SubmissionError = InferError<typeof FormError.Submission>;
...
function validateSignupForm(data: unknown): Result<SignupForm, ValidationError> {
  ...
  if (!data || typeof data !== 'object') {
    return FormError.Validation({ receivedType: typeof data });
  }
  ...
  if (!email || !email.includes('@')) {
    return FormError.Validation({ field: 'email', value: email });
  }

  // Password validation
  if (!password || password.length < 8) {
    return FormError.Validation({ field: 'password' });
  }

  // Password match validation
  if (password !== confirmPassword) {
    return FormError.Validation({ field: 'confirmPassword' });
  }
