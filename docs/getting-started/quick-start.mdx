...
const { ParseError, ParseErr } = createTaggedError("ParseError")
  .withMessage(() => 'Input is not a valid number');
...
const { JsonError, JsonErr } = createTaggedError("JsonError")
  .withFields<{ text: string }>()
  .withMessage(({ text }) => `Failed to parse JSON: ${text}`);
...
const { ApiError, ApiErr } = createTaggedError("ApiError")
  .withFields<{ endpoint: string; statusCode?: number }>()
  .withMessage(({ endpoint }) => `API request failed: ${endpoint}`);
...
// Define our error types with factories
const { ValidationError, ValidationErr } = createTaggedError("ValidationError")
  .withFields<{ field?: string; value?: string; receivedType?: string }>()
  .withMessage(({ receivedType, field }) => {
...
const { SubmissionError, SubmissionErr } = createTaggedError("SubmissionError")
  .withFields<{ email: string }>()
  .withMessage(({ email }) => `Failed to create account for ${email}`);
type SubmissionError = ReturnType<typeof SubmissionError>;
...
// Create tagged errors with factories
const { MyError, MyErr } = createTaggedError("MyError")
  .withFields<{ /* structured data */ }>()
  .withMessage(({ /* fields */ }) => "Something went wrong");
type MyError = ReturnType<typeof MyError>;
const error = MyError({ /* structured data */ });
