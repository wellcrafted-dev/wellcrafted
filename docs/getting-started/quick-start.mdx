...
const { ParseError, ParseErr } = errors;
type ParseError = InferError<typeof errors, 'ParseError'>;
...
const jsonErrors = defineErrors({
  JsonError: ({ text }: { text: string }) => ({
    message: `Failed to parse JSON: ${text}`,
    text,
  }),
});
const { JsonError, JsonErr } = jsonErrors;
type JsonError = InferError<typeof jsonErrors, 'JsonError'>;

function parseJson<T>(text: string): Result<T, JsonError> {
  return trySync({
    try: () => JSON.parse(text),
    catch: () => JsonErr({ text: text.substring(0, 100) }) // Truncate for logging
  });
}
...
// Define error with typed fields - endpoint is always useful for API errors
const apiErrors = defineErrors({
  ApiError: ({ endpoint, statusCode }: { endpoint: string; statusCode?: number }) => ({
...
type ApiError = InferError<typeof apiErrors, 'ApiError'>;
...
async function fetchUser(id: number): Promise<Result<User, ApiError>> {
  ...
  return tryAsync({
    ...
    catch: () => ApiErr({ endpoint })  // TypeScript enforces this!
  });
}
...
const formErrors = defineErrors({
  ValidationError: ({ field, value, receivedType }: { field?: string; value?: string; receivedType?: string }) => ({
    message: (() => {
  ...
  SubmissionError: ({ email }: { email: string }) => ({
    message: `Failed to create account for ${email}`,
    email,
  }),
});
const { ValidationError, ValidationErr, SubmissionError, SubmissionErr } = formErrors;
type ValidationError = InferError<typeof formErrors, 'ValidationError'>;
type SubmissionError = InferError<typeof formErrors, 'SubmissionError'>;
...
function validateSignupForm(data: unknown): Result<SignupForm, ValidationError> {
  ...
  if (!data || typeof data !== 'object') {
    return ValidationErr({ receivedType: typeof data });
  }
  ...
  if (!email || !email.includes('@')) {
    return ValidationErr({ field: 'email', value: email });
  }

  // Password validation
  if (!password || password.length < 8) {
    return ValidationErr({ field: 'password' });
  }

  // Password match validation
  if (password !== confirmPassword) {
    return ValidationErr({ field: 'confirmPassword' });
  }
...
async function submitSignup(
  ...
  return tryAsync({
    ...
    catch: () => SubmissionErr({ email: form.email })
  });
}
...
const errors = defineErrors({
  MyError: ({ /* fields */ }: { /* types */ }) => ({
    message: "Something went wrong",
    /* ...fields */
  }),
});
const { MyError, MyErr } = errors;
type MyError = InferError<typeof errors, 'MyError'>;
const error = MyError({ /* fields */ });
