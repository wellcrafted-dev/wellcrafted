...
type RecorderServiceError = Readonly<{ name: 'RecorderServiceError'; message: string }>;
type TranscriptionServiceError = Readonly<{ name: 'TranscriptionServiceError'; message: string }>;
type ClipboardServiceError = Readonly<{ name: 'ClipboardServiceError'; message: string }>;

// Application-level errors for UI
type WhisperingError = Readonly<{ name: 'WhisperingError'; message: string }>;
```
...
const apiErrors = defineErrors({
  ApiError: (fields: Record<string, unknown>) => ({
...
These aren't theoretical benefits – they're proven in a production application that users depend on daily. Whether you're building a simple service or a complex multi-platform application, wellcrafted's patterns provide the foundation for robust, maintainable code.
