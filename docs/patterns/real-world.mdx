...
// Service-specific errors with typed context
const { AuthServiceError, AuthServiceErr } = createTaggedError("AuthServiceError")
  .withContext<{ operation: string; email?: string; userId?: string; tokenPrefix?: string }>()
  .withMessage(({ context }) => `Auth ${context.operation} failed`);
  ...
  return {
    async login(
      ...
      const userResult = await tryAsync({
        ...
        catch: (error) => AuthServiceErr({
          context: { operation: "login", email: email.substring(0, email.indexOf('@')) + '@***' },
          cause: error
        })
      });
      ...
      const tokenResult = await tryAsync({
        try: () => tokenService.generate(userResult.data.id),
          ...
          context: { operation: "login", userId: userResult.data.id },
          cause: error
        })
      });
    ...
    async validateSession(
      ...
      const validationResult = await tryAsync({
        try: () => tokenService.validate(token),
          ...
          context: { operation: "validateSession", tokenPrefix: token.substring(0, 8) + '...' },
          cause: error
        })
      });
      ...
      const userResult = await tryAsync({
        try: () => db.users.findById(validationResult.data.userId),
          ...
          context: { operation: "validateSession", userId: validationResult.data.userId },
          cause: error
        })
      });
...
// Service errors with typed context
const { FileServiceError, FileServiceErr } = createTaggedError("FileServiceError")
  ...
  .withMessage(({ context }) => `File ${context.operation} failed`);
    ...
    async uploadFile(
      ...
      const metadataResult = await tryAsync({
        try: () => db.files.create({
          ...
          context: { operation: "upload", fileId, fileName: file.name },
          cause: error
        })
      });
    ...
    async getFile(
      ...
      const fileResult = await tryAsync({
        try: () => db.files.findById(fileId),
          ...
          context: { operation: "getFile", fileId, userId },
          cause: error
        })
      });
      ...
      if (fileResult.data.userId !== userId) {
        return FileServiceErr({
          context: { operation: "getFile", fileId, userId, ownerId: fileResult.data.userId },
        });
      }
...
// Form validation errors with typed context
const { ValidationError, ValidationErr } = createTaggedError("ValidationError")
  .withContext<{ field?: string; value?: unknown; receivedValue?: unknown; allErrors?: Array<{ path: string; message: string }> }>()
  .withMessage(({ context }) => context.field ? `Validation failed on ${context.field}` : 'Validation failed');
