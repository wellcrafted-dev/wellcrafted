...
const { AuthServiceError, AuthServiceErr } = createTaggedError("AuthServiceError")
  .withFields<{ operation: string; email?: string; userId?: string; tokenPrefix?: string }>()
  .withMessage(({ operation }) => `Auth ${operation} failed`);
...
const { FileServiceError, FileServiceErr } = createTaggedError("FileServiceError")
  ...
  .withMessage(({ operation }) => `File ${operation} failed`);
...
const { ValidationError, ValidationErr } = createTaggedError("ValidationError")
  .withFields<{ field?: string; value?: unknown; receivedValue?: unknown; allErrors?: Array<{ path: string; message: string }> }>()
  .withMessage(({ field }) => field ? `Validation failed on ${field}` : 'Validation failed');
