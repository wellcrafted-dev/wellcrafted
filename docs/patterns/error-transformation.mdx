...
const { RecorderServiceError, RecorderServiceErr } = createTaggedError('RecorderServiceError')
  .withFields<{ userAgent?: string; permissions?: string; errorCode?: string }>()
...
const { DbServiceError, DbServiceErr } = createTaggedError('DbServiceError')
  .withFields<{ operation: string; table?: string; query?: string; timestamp?: string }>()
  .withMessage(({ operation }) => `DB ${operation} failed`);
