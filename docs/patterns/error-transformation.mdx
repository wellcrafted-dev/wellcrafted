...
Services return detailed, domain-specific errors using `defineErrors`:
...
const recorderErrors = defineErrors({
  RecorderServiceError: ({ userAgent, permissions, errorCode }: { userAgent?: string; permissions?: string; errorCode?: string }) => ({
...
const dbErrors = defineErrors({
  DbServiceError: ({ operation, table, query, timestamp }: { operation: string; table?: string; query?: string; timestamp?: string }) => ({
...
const { DbServiceError, DbServiceErr } = dbErrors;
type DbServiceError = InferError<typeof dbErrors, 'DbServiceError'>;
