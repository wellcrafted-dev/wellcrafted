...
Services return detailed, domain-specific errors using `defineError`:

```typescript
import { defineError } from 'wellcrafted/error';
import { tryAsync, Result } from 'wellcrafted/result';

const { RecorderServiceError, RecorderServiceErr } = defineError('RecorderServiceError');
...
export const recorder = {
  startRecording: defineMutation({
    ...
    resultMutationFn: async (): Promise<Result<void, UIError>> => {
...
```typescript
// services/db.ts
const { DbServiceError, DbServiceErr } = defineError('DbServiceError');
...
export const recordings = {
  getAllRecordings: defineQuery({
    queryKey: ['recordings'],
    resultQueryFn: async (): Promise<Result<Recording[], RecordingUIError>> => {
  ...
  createRecording: defineMutation({
    ...
    resultMutationFn: async (recording: Recording): Promise<Result<Recording, RecordingUIError>> => {
...
export const auth = {
  signIn: defineMutation({
    resultMutationFn: async (credentials) => {
  ...
  changePassword: defineMutation({
    resultMutationFn: async (passwords) => {
...
const validateInput = defineMutation({
  resultMutationFn: async (input): Promise<Result<ValidatedInput, UIError>> => {
...
const processBatch = defineMutation({
  resultMutationFn: async (items): Promise<Result<BatchResult, UIError>> => {
...
export const getUser = defineQuery({
  resultQueryFn: () => services.getUser(id), // Missing error transformation!
});
```
