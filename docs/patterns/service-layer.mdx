...
const { UserServiceError, UserServiceErr } = createTaggedError("UserServiceError")
  ...
  .withMessage(({ context }) => `User ${context.operation} failed`);
    ...
    async getUser(id: string): Promise<Result<User, UserServiceError>> {
      ...
      const result = await tryAsync({
        try: () => db.users.findById(id),
          ...
          context: { userId: id, operation: "getUser" }
        })
      });
...
const { NotificationServiceError, NotificationServiceErr } =
  createTaggedError("NotificationServiceError")
    .withContext<{ action: string; platform?: string }>()
    .withMessage(({ context }) => `Notification ${context.action} failed`);
    ...
    async notify(message) {
      return tryAsync({
        try: () => {
          ...
          context: { action: "notify", platform: "desktop" }
        })
      });
    },
    ...
    async notify(message) {
      ...
      if (permission !== 'granted') {
        return NotificationServiceErr({
          context: { action: "notify", platform: "web" }
        });
      }
...
Always include relevant debugging information. Define your error with `.withContext()` to require it:
...
// Now context is required with the exact shape
return UserServiceErr({
  context: {
    userId,
...
Transform low-level errors into domain-specific ones. Use `.withCause()` to create error hierarchies:

```typescript
// Define errors with typed causes for layered architectures
...
if (dbResult.error) {
  // Transform to service-level error, wrapping the database error
  return UserServiceErr({
    context: { userId },
...
```typescript
// Define the error type with required context
const { UserServiceError, UserServiceErr } = createTaggedError("UserServiceError")
  .withContext<{ userId: string; operation: string }>()
  .withMessage(({ context }) => `User ${context.operation} failed`);
type UserServiceError = ReturnType<typeof UserServiceError>;
