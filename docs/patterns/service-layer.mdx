...
const userErrors = defineErrors({
  UserServiceError: ({ userId, operation }: { userId?: string; operation: string }) => ({
    message: `User ${operation} failed`,
    userId,
    operation,
  }),
});
const { UserServiceError, UserServiceErr } = userErrors;
...
const orderErrors = defineErrors({
  PaymentServiceError: ({ amount, paymentId }: { amount?: number; paymentId?: string }) => ({
    message: 'Payment processing failed',
    amount,
    paymentId,
  }),
  OrderServiceError: ({ input }: { input?: OrderInput }) => ({
    message: 'Order operation failed',
    input,
  }),
});
const { PaymentServiceError, PaymentServiceErr, OrderServiceError, OrderServiceErr } = orderErrors;

type PaymentServiceError = InferError<typeof orderErrors, 'PaymentServiceError'>;
type OrderServiceError = InferError<typeof orderErrors, 'OrderServiceError'>;
...
export function createOrderService(
  ...
  return {
    async createOrder(input: OrderInput): Promise<Result<Order, OrderServiceError | PaymentServiceError>> {
...
const notificationErrors = defineErrors({
  NotificationServiceError: ({ action, platform }: { action: string; platform?: string }) => ({
    message: `Notification ${action} failed`,
    action,
    platform,
  }),
});
const { NotificationServiceError, NotificationServiceErr } = notificationErrors;

type NotificationServiceError = InferError<typeof notificationErrors, 'NotificationServiceError'>;
  ...
  return {
    async notify(message) {
      return tryAsync({
        ...
        catch: () => NotificationServiceErr({
          action: "notify", platform: "desktop"
        })
      });
    },
...
const fileErrors = defineErrors({
  FileServiceError: ({ path }: { path: string }) => ({
...
describe('UserService', () => {
  ...
  it('should reject invalid email', async () => {
    ...
    expect(result.error?.message).toBe('User createUser failed');
  });
});
```
...
### 2. Include Fields in Errors

Always include relevant debugging information. Define your error constructor to require it:

```typescript
const errors = defineErrors({
  UserServiceError: ({ userId, operation, timestamp }: {
    userId: string;
    operation: string;
    timestamp?: string;
  }) => ({
...
// Now fields are required with the exact shape
return UserServiceErr({
  userId,
...
const userErrors = defineErrors({
  UserServiceError: ({ userId }: { userId: string }) => ({
    message: `User service operation failed for ${userId}`,
    userId,
  }),
});
const { UserServiceError, UserServiceErr } = userErrors;
...
const dbResult = await tryAsync({
  try: () => db.query(sql),
  catch: () => DatabaseErr({ sql })
});

if (dbResult.error) {
  // Transform to service-level error
  return UserServiceErr({ userId });
}
```
...
```typescript
// Define the error type with required fields
const errors = defineErrors({
  UserServiceError: ({ userId, operation }: { userId: string; operation: string }) => ({
    message: `User ${operation} failed`,
    userId,
...
const { UserServiceError, UserServiceErr } = errors;
type UserServiceError = InferError<typeof errors, 'UserServiceError'>;
