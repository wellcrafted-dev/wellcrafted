...
const UserServiceError = defineErrors({
  Operation: ({ userId, operation }: { userId?: string; operation: string }) => ({
  ...
  return {
    async getUser(id: string): Promise<Result<User, UserServiceError>> {
      ...
      const result = await tryAsync({
        ...
        catch: () => UserServiceError.Operation({
    ...
    async createUser(input: CreateUserInput): Promise<Result<User, UserServiceError>> {
      if (!input.email.includes('@')) {
        return UserServiceError.Operation({ operation: "createUser" });
      }

      return tryAsync({
        try: () => db.users.create(input),
        catch: () => UserServiceError.Operation({ operation: "createUser" })
      });
    },
...
const OrderError = defineErrors({
  Payment: ({ amount, paymentId }: { amount?: number; paymentId?: string }) => ({
    message: 'Payment processing failed',
    amount,
    paymentId,
  }),
  Db: ({ input }: { input?: OrderInput }) => ({
    message: 'Order operation failed',
    input,
  }),
});

type PaymentError = InferError<typeof OrderError.Payment>;
type OrderDbError = InferError<typeof OrderError.Db>;
  ...
  return {
    async createOrder(input: OrderInput): Promise<Result<Order, OrderDbError | PaymentError>> {
      ...
      const paymentResult = await tryAsync({
        ...
        catch: () => OrderError.Payment({ amount: input.payment.amount })
      });
...
const NotificationError = defineErrors({
  Operation: ({ action, platform }: { action: string; platform?: string }) => ({
  ...
  return {
    async notify(message) {
      return tryAsync({
        ...
        catch: () => NotificationError.Operation({ action: "notify", platform: "desktop" })
      });
    },
  ...
  return {
    async notify(message) {
      ...
      if (permission !== 'granted') {
        return NotificationError.Operation({ action: "notify", platform: "web" });
      }
      ...
      return tryAsync({
        ...
        catch: () => NotificationError.Operation({ action: "notify", platform: "web" })
      });
    },
...
const UserServiceError = defineErrors({
  Operation: ({ userId }: { userId: string }) => ({
...
if (dbResult.error) {
  ...
  return UserServiceError.Operation({ userId });
}
```
...
export function createUserService(db: Database) {
  return {
    async getUser(id: string): Promise<Result<User, UserServiceError>> {
      return tryAsync({
        try: () => db.users.findById(id),
        catch: () => UserServiceError.Operation({ userId: id, operation: "getUser" })
      });
    }
  };
}
