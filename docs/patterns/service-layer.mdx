...
const { UserServiceError, UserServiceErr } = createTaggedError("UserServiceError")
  ...
  .withMessage(({ operation }) => `User ${operation} failed`);
...
const { PaymentServiceError, PaymentServiceErr } = createTaggedError("PaymentServiceError")
  .withFields<{ amount?: number; paymentId?: string }>()
  .withMessage(() => 'Payment processing failed');
const { OrderServiceError, OrderServiceErr } = createTaggedError("OrderServiceError")
  .withFields<{ input?: OrderInput }>()
  .withMessage(() => 'Order operation failed');

type PaymentServiceError = ReturnType<typeof PaymentServiceError>;
type OrderServiceError = ReturnType<typeof OrderServiceError>;
...
const { NotificationServiceError, NotificationServiceErr } =
  createTaggedError("NotificationServiceError")
    .withFields<{ action: string; platform?: string }>()
    .withMessage(({ action }) => `Notification ${action} failed`);
...
const { FileServiceError, FileServiceErr } = createTaggedError("FileServiceError")
  ...
  .withMessage(({ path }) => `File operation failed: ${path}`);
...
const { UserServiceError } = createTaggedError("UserServiceError")
  .withMessage(() => 'User service operation failed');
...
Always include relevant debugging information. Define your error with `.withFields()` to require it:
...
const { DatabaseError, DatabaseErr } = createTaggedError("DatabaseError")
  .withFields<{ sql: string }>()
  .withMessage(({ sql }) => `Database query failed: ${sql}`);
type DatabaseError = ReturnType<typeof DatabaseError>;

const { UserServiceError, UserServiceErr } = createTaggedError("UserServiceError")
  .withFields<{ userId: string }>()
  .withMessage(({ userId }) => `User service operation failed for ${userId}`);
