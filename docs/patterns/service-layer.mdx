...
```typescript
const { PaymentServiceError, PaymentServiceErr } = defineError("PaymentServiceError");
const { EmailServiceError, EmailServiceErr } = defineError("EmailServiceError");
...
const { NotificationServiceError, NotificationServiceErr } = 
  defineError("NotificationServiceError");
