...
This creates a **discriminated union** where the `error` property acts as the sole discriminant. When in doubt, check the error:

- `error === null` → **Ok**
- `error !== null` → **Err**

Why not check `data`? Because `data` can legitimately be `null` as a success value (`Ok<null>`), making it unreliable as a discriminant.
