...
This page reveals the simplicity behind wellcrafted. Understanding the implementation helps you appreciate why the library is so lightweight.
...
**That's it!** The entire foundation is built on this simplicity. Let's break down why this design works.

## The Discriminated Union Pattern

The behavior comes from how TypeScript's type system interprets this structure:

- **`Ok<T>`** always has `data: T` and `error: null`
- **`Err<E>`** always has `error: E` and `data: null`  
- **`Result<T, E>`** is simply `Ok<T> | Err<E>`

This creates a **discriminated union** where `error` is the reliable discriminator:

- `error === null` → **Ok** (data is `T`)
- `error !== null` → **Err** (error is `E`)

**Pro tip**: Check `error` with an exact null comparison. `error === null` means success; `error !== null` means failure. Checking `data` works only when your success type is non-nullable, but `error` handles all cases, including `Ok<null>` where the success value itself is `null`.
...
The design enables the destructuring pattern:
...
This simple foundation enables more patterns:
...
3. **Stay Transparent**: No hidden behavior
4. **Keep It Simple**: Complexity should be in your business logic, not your tools

> **The beauty is in the transparency** - you can see exactly how it works under the hood, yet it provides type safety and ergonomics.
...
<Note>
Want to see how this simple foundation enables more error handling patterns? Check out the [Result Pattern Deep Dive](/core/result-pattern) or explore [real-world examples](/patterns/real-world).
</Note>
