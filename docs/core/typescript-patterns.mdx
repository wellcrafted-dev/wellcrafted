...
type NetworkError = InferError<typeof UserError.Network>;
type AuthError = InferError<typeof UserError.Auth>;
...
export async function updateUserProfile(
  ...
  if (!isAuthenticated()) {
    return UserError.Auth();
  }

  // Network operation
  return tryAsync({
    try: () => api.updateUser(userId, updates),
    catch: (error) => UserError.Network({
      cause: error,
    })
  });
}
  ...
  if (result.error) {
    ...
    switch (result.error.name) {
      ...
      case 'Network':
        ...
        break;
      case 'Auth':
        // TypeScript knows this is the Auth variant
        redirectToLogin();
        break;
      default:
