...
const errors = defineErrors({
  ValidationError: ({ message }: { message: string }) => ({ message }),
  NetworkError: ({ message }: { message: string }) => ({ message }),
  AuthError: ({ message }: { message: string }) => ({ message }),
});
const { ValidationError, ValidationErr, NetworkError, NetworkErr, AuthError, AuthErr } = errors;

type ValidationError = InferError<typeof errors, 'ValidationError'>;
type NetworkError = InferError<typeof errors, 'NetworkError'>;
type AuthError = InferError<typeof errors, 'AuthError'>;
