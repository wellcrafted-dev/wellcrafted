...
const UserError = defineErrors({
  Validation: ({ message }: { message: string }) => ({ message }),
  Network: ({ message }: { message: string }) => ({ message }),
  Auth: ({ message }: { message: string }) => ({ message }),
});
  ...
  return {
    async transcribe(blob, options) {
      return tryAsync({
        ...
        catch: (error) => TranscriptionError.Operation({
  ...
  mutationFn: async (
    ...
    switch (provider) {
      ...
      default: {
        ...
        return TranscriptionError.Operation({
          provider,
        });
      }
    }
  ...
  return {
    async create(input) {
      return tryAsync({
        ...
        catch: (error) => DatabaseError.Operation({
