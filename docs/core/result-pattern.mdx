...
The key insight: **The `error` property is the sole discriminant**. When in doubt, check the error.

- `error === null` → **Ok**: TypeScript knows `data` is `T`
- `error !== null` → **Err**: TypeScript knows `error` is `E`

### Why `error` is the Universal Discriminant

While you might think `data` could also serve as a discriminant, it's not universal:

```typescript
// ✅ error is ALWAYS reliable
if (result.error === null) { /* Always Ok */ }
if (result.error !== null) { /* Always Err */ }

// ⚠️ data is NOT always reliable  
if (result.data !== null) { /* Ok - but only if T doesn't include null */ }
if (result.data === null) { /* Ambiguous! Could be Ok<null> OR Err<E> */ }
```

The issue: `data` can legitimately be `null` as a success value (`Ok<null>`). So checking `data === null` doesn't tell you whether you have a success with null data or an error.

**Best practice**: Always check `error`, even if you know your success type doesn't include `null`. Consistency makes code more maintainable.
