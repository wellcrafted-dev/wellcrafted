...
// - NetworkError({ context?, cause?, message? }): Creates plain error object
// - NetworkErr({ context?, cause?, message? }): Creates Err-wrapped error (for Result returns)
```
...
You can override the auto-computed message at any call site:

```typescript
// Override message for a specific call site
FileErr({
  context: { path: '/etc/passwd', operation: 'write' },
  message: 'Permission denied'  // Overrides the template
});
```
...
The factory pattern works seamlessly with error mapping. Pass `context` and `cause` at the call site — `message` is auto-computed:
