...
wellcrafted provides a powerful `defineError` factory function that eliminates boilerplate and provides two convenient ways to create errors:
...
const { UserNotFoundError, UserNotFoundErr } = defineError('UserNotFoundError');
const { ValidationError, ValidationErr } = defineError('ValidationError');
const { DatabaseError, DatabaseErr } = defineError('DatabaseError');
