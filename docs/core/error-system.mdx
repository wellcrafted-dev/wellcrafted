...
When `.withMessage()` is used, the template **seals** the message — `message` is not in the factory input type. The template computes the message from the fields:
...
wellcrafted provides a fluent `createTaggedError` builder that eliminates boilerplate and enforces consistent error structure. Factories are available immediately — `.withMessage(fn)` is an **optional** step that seals the message when present.
...
### `.withMessage(fn)` — Optional, Seals the Message

`.withMessage(fn)` is an **optional** builder step. It accepts a function that receives the fields and returns the error message string. When present, `message` is NOT in the factory input type — the template owns the message entirely.
...
// Without .withMessage() — message required at call site
const { GenericError, GenericErr } = createTaggedError('GenericError')
  .withFields<{ path: string }>();
...
<Note>
Two mutually exclusive modes: without `.withMessage()`, `message` is required at the call site. With `.withMessage()`, the template seals the message — `message` is not in the input type.
</Note>

### `.withFields<T>()` — Defining Structured Data

Use `.withFields<T>()` to define additional fields spread flat on the error object. The type `T` must be a `JsonObject` and must not include the reserved key `name`.
...
The factory pattern works seamlessly with error mapping. When `.withMessage()` is used, pass only fields — the message is sealed by the template:
