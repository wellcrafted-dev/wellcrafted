...
wellcrafted's error system is built on a simple principle: **errors should be data, not control flow**. This page explains the design philosophy, implementation details, and best practices for error handling in your applications.
...
At the heart of wellcrafted's error system is the tagged error: a structured, serializable error representation that works with TypeScript's type system.
...
The factory pattern works with error mapping. Each variant returns `Err<...>` directly, making it a natural fit for the `catch` callback:
...
function handleForm(data: unknown): Result<User, ValidationError | SubmissionError> { ... }
```
...
async function fetchWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    const result = await tryAsync({
      try: (): Promise<T> => fetch(url).then(r => r.json()),
...
TaggedErrors are designed to work with the Result type:
