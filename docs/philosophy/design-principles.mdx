...
wellcrafted is built on four core principles that guide every design decision. These aren't abstract ideals; they're practical philosophies proven in 22,824 lines of production TypeScript code.
...
if (error) {
  ...
  switch (error.name) {
    case "ValidationError":
      showValidationMessages(error.fields);
      break;
    case "AuthError":
      redirectToLogin();
      break;
...
- **Type safety**: TypeScript lets you enforce that every error case is handled
...
JavaScript gets a lot of criticism, but it has some genuinely useful features that wellcrafted embraces:
...
**No Method Chaining**: Rust's `.map()` and `.and_then()` read well in Rust, but feel foreign in JavaScript. We use standard JavaScript control flow instead.
...
**No Hidden Behavior**: Every pattern is explicit and visible. No concealed behavior, no surprising transformations.
...
const results = await Promise.all([
  tryAsync({ try: () => fetchUser(id1), catch: handleUserError }),
  tryAsync({ try: () => fetchUser(id2), catch: handleUserError }),
  tryAsync({ try: () => fetchUser(id3), catch: handleUserError }),
]);
...
**`defineErrors`**: Define structured, discriminated error data
...
wellcrafted's design principles aren't academic abstractions; they're practical philosophies proven in production code. By treating errors as values, working with JavaScript's strengths, making behavior explicit, and favoring composition over complexity, we create code that is:
