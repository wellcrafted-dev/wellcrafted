...
wellcrafted's error-handling patterns weren't designed in isolation; they evolved from real production pain points where traditional exception handling fell short.
...
**How long to detect?** Days or weeks. Silent failures are the worst kind: they break user trust without triggering your monitoring systems.
...
type GatewayError = Readonly<{
  ...
  customerId: string;
  gatewayResponse: number | undefined;
  timestamp: string;
}>;
  ...
  const { data: result, error } = await tryAsync({
    try: () => paymentGateway.charge({
      ...
      Err<GatewayError>({
        ...
        paymentMethodId: paymentMethod.id,
        customerId: order.customerId,
        gatewayResponse: (error as { response?: { status?: number } }).response?.status,
...
**What changed**: The calling code *must* handle the error case. TypeScript enforces this: you can't access `profile` without first checking if `error` is null. Silent failures become impossible.

### Scenario 3 Solved: Discriminated Error Types

```typescript
type ApiError =
  | Readonly<{ name: "RateLimitError"; message: string; retryAfter: string | null }>
  | Readonly<{ name: "NetworkError"; message: string }>
  | Readonly<{ name: "AuthError"; message: string }>
  | Readonly<{ name: "NotFoundError"; message: string }>;
...
async function fetchUserData(
  ...
  return await tryAsync({
    try: async () => {
      ...
      const e = error as { type?: string; retryAfter?: string | null; status?: number };
      switch (e.type) {
        case "rate_limit":
          return Err({
        ...
        default:
          return Err({
            ...
            message: e.type ? "Network request failed" : "Unexpected error",
            userId: id,
            status: e.status,
          });
      }
...
if (error.name === "Auth") {
  return WhisperingError.ApiKeyMissing({
...
function handleApiError(error: ApiError) {
  switch (error.name) {
    case "RateLimitError":
      return showRetryAfterDelay(error.retryAfter);
    case "AuthError":
      return redirectToLogin();
    case "NetworkError":
      return showNetworkErrorDialog();
    default: {
      // every variant is handled, so `error` is `never` here;
      // add a variant without a case and this line stops compiling
...
**Team impact**: Code reviews can mechanically verify that all error cases are handled. No more "did we handle the timeout case?" discussions: the `never` check in `default` enforces completeness.
...
wellcrafted's consistent error structure enables structured monitoring:
...
Production systems taught us that error handling isn't just about catching exceptions: it's about building systems that fail gracefully, provide actionable feedback, and maintain observability under stress.
...
These aren't theoretical concerns: they're the difference between a system that scales gracefully and one that requires constant manual intervention.
